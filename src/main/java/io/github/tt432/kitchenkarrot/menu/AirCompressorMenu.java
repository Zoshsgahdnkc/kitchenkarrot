...
public class AirCompressorMenu extends KKBeMenu<AirCompressorBlockEntity> {
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index < playerSlotSize) {
            if (slotItem.is(ModItems.EMPTY_CAN.get())
                    && slots.get(playerSlotSize + 4).safeInsert(slotItem).isEmpty())
                return ItemStack.EMPTY;
            if (slotItem.is(Items.REDSTONE)
                    && slots.get(playerSlotSize + 5).safeInsert(slotItem).isEmpty())
                return ItemStack.EMPTY;
